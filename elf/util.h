...
unsigned char * MapFile(HANDLE hFile, off_t start, off_t size, DWORD flag, LPVOID baseAddr) {
    ...
    unsigned char * data = (unsigned char *) ::MapViewOfFileEx(hFileMapping, FILE_MAP_COPY | FILE_MAP_EXECUTE, 0, start, size, 0);
    if (!data) {
        PrintError();
        return 0;
    }
    ...
    unsigned char * ptr = (unsigned char *) VirtualAlloc((LPVOID) drealpos, realsize, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
    if (!ptr) {
        PrintError();
        return 0;
    }
